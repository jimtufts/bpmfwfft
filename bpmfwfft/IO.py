...
class PrmtopLoad(object):
    ...
    def _conv_pointers_list_2_dict(self):
        ...
        self._parameters["POINTERS"] = {pointers_key[i]: self._parameters["POINTERS"][i] for i in
                                        range(len(pointers_key))}
        return None
    ...
    def _set_LJ_grid_charges(self):
        ...
        for i in range(NTYPES):
            ...
            if self._parameters["LENNARD_JONES_ACOEF"][LJ_index] < 1.0e-6:
                ...
                factor = 2. * self._parameters["LENNARD_JONES_ACOEF"][LJ_index] / \
                         self._parameters["LENNARD_JONES_BCOEF"][LJ_index]
                LJ_diameter[i] = pow(factor, 1. / 6)  # originally 1./6
    ...
    def get_parm_for_grid_calculation(self):
        parm = dict()
        keys = ["CHARGE_E_UNIT", "R_LJ_CHARGE", "A_LJ_CHARGE", "LJ_SIGMA", "LJ_EPSILON", "VDW_RADII", "POINTERS",
                "MASS", "PDB_TEMPLATE", "BONDS_WITHOUT_HYDROGEN", "BONDS_INC_HYDROGEN"]
        for key in keys:
...
if __name__ == "__main__":
    ...
    # bonds = {}
    # for key,value in prmtop_obj.get_parm_for_grid_calculation().items():
    #     if key == "BONDS_WITHOUT_HYDROGEN":
    #         for i in np.arange(len(value), step=3):
    #             if i not in list(bonds.keys()):
    #                 bonds[(value[i]/3)+1] = []
    #                 bonds[(value[i]/3)+1].append((value[i+1]/3)+1)
    #             else:
    #                 bonds[(value[i] / 3) + 1].append((value[i + 1] / 3) + 1)
    # print(bonds)
    # crd = inpcrd_obj.get_coordinates()
    # b = []
    ...
    # for b_s in b:
    #     print(midpoint(crd[int(b_s[0])-1], crd[int(b_s[1])-1]))

    bonds = {}
    prmtop_parms = prmtop_obj.get_parm_for_grid_calculation()
    for key, value in prmtop_parms.items():
        if key == "BONDS_INC_HYDROGEN":
            for i in np.arange(len(value), step=3):
    ...
    # c = np.unique(np.array(b), return_counts=True)
    for b_s in b:
        print(midpoint(crd[int(b_s[0])], crd[int(b_s[1])]))
